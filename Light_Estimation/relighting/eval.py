### model for relighting evaluation ###
...
def main(opts):
  ...
  sample = {
    "input": _preprocess(lightEstim.query_images(opts.scene, dirs=src, mip=3, hdr=True)[0]),
    "target": _preprocess(lightEstim.query_images(opts.scene, dirs=tgt, mip=3, hdr=True)[0])
  }
  with th.no_grad():
...
def _save(data, fname, makedirs=True):
  ...
  lightEstim.writeimage(npdata, fname)

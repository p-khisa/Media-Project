...
class Relighter(nn.Module):
  ...
  2018-04-08
  """
  def __init__(self, width=40, n_in=1, n_out=1, normals=False): # 40 was 64
    ...
    self.autoencoder = Autoencoder(
        n_in, n_out, num_levels=4, #was 8
        increase_factor=1,# was 2
        num_convs=2, width=w, ksize=2, #was 3
        activation="relu",
        pooling="max",
        output_type="linear")
  ...
  def forward(self, samples):
    ...
    if "normals" in samples.keys() and self.normals:
      im = th.cat([im, samples["normals"]], -1)#originally was 3
